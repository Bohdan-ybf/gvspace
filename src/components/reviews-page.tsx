  ...
  return (
    <main className="reviews-page">
        ...
        <div className="container reviews-hero-content">
          ...
          <div className="reviews-hero-heading">
            <h1>
              {uk ? (
                <>
                  Бізнеси, які обрали систему
                  <br />
                  замість лотереї
                </>
              ) : (
                <>
                  Businesses that chose a system
                  <br />
          ...
          <dl>
            <div>
              <dd>50+</dd>
              <dt>{uk ? "ПРОЄКТІВ ЗАВЕРШЕНО" : "PROJECTS COMPLETED"}</dt>
            </div>
            <div>
              <dd>$10M+</dd>
              <dt>{uk ? "БЮДЖЕТІВ ПІД УПРАВЛІННЯМ" : "BUDGETS MANAGED"}</dt>
            </div>
            <div>
              <dd>4</dd>
              <dt>{uk ? "НАПРЯМКИ ПОСЛУГ" : "SERVICE DIRECTIONS"}</dt>
            </div>
            <div>
              <dd>87%</dd>
              <dt>{uk ? "КЛІЄНТІВ ПОВЕРТАЮТЬСЯ" : "CLIENTS RETURN"}</dt>
            </div>
          </dl>
        </div>
      </section>
      ...
      <section className="container review-clients">
        <span className="mono">
          {uk ? "КОМПАНІЇ, ЯКІ НАМ ДОВІРИЛИСЬ" : "COMPANIES THAT TRUSTED US"}
        </span>
        <div>
          {(companies.length
            ? companies
